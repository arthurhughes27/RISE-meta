...
df <- hipc_merged_all_noNorm %>%
  ...
  filter(length(unique(participant_id)) >= max(sample_sizes)) %>% # Filter out studies with less than 20 participants
  ungroup() %>%
  select(
...
fpr_df = fpr_df %>%
  mutate(n_within_study = factor(n_within_study, levels = sample_sizes))
...
p1 = ggplot(fpr_df, aes(x = n_within_study, y = fpr)) +
  ...
  labs(
    x = "Number of samples within studies",
