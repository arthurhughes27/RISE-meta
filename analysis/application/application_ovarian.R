...
jointModel_plot <- trial_effects %>%
  ...
  scale_x_continuous(limits = c(plot.min.global, max(trial_effects$u.s, na.rm = TRUE) + 0.2),
                     expand = c(0, 0)) +
  scale_y_continuous(limits = c(plot.min.global, max(trial_effects$u.y, na.rm = TRUE) + 0.2),
                     expand = c(0, 0)) +
  coord_fixed(ratio = 1) +
...
combined_plot <- jointModel_plot_mod +
  ...
  plot_annotation(title = "Comparison of Trial-level Surrogacy Methods Applied to Ovarian cancer data",
                  theme = theme(plot.title = element_text(
                    size = 29, face = "bold", hjust = 0.5
                  )))
