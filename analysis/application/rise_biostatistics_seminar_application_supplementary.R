...
train_indices <- hipc_merged_all_noNorm_filtered %>%
  ...
  group_by(study_accession) %>%
  slice_sample(prop = 1) %>%                # sample 66% of participants per study
  ungroup()
...
# primary post timepoint (preferred) and fallback post timepoint
tp <- 1L
fallback_tp <- 2L
...
hipc_merged_all_noNorm_filtered <- hipc_merged_all_noNorm %>%
  ...
  # keep only participants with exactly one pre and exactly one chosen post,
  # and keep only the rows at time 0 and at the chosen post time
  filter(
    has_pre == 1,
    !is.na(desired_post),
    study_time_collected %in% c(0L, desired_post)
  ) %>%
  # ensure we actually have exactly one pre row and exactly one post row left
  filter(
    sum(study_time_collected == 0L) == 1,
    sum(study_time_collected == desired_post) == 1
  ) %>%
  ungroup() %>%
  # keep studies with >2 distinct participants
  group_by(study_accession) %>%
...
p4 <- ggplot(df_plot, aes(x = mu.delta, y = marker)) +
  geom_segment(aes(x = ci.delta.lower, xend = ci.delta.upper, y = marker, yend = marker, color = sig_cat),
               size = 1.1, lineend = "round") +
  geom_point(aes(color = sig_cat), size = 4) +
  geom_vline(xintercept = c(-epsilon_val, epsilon_val), linetype = "dashed", color = "red", linewidth = 0.8) +
